...
def test_health_metadata_graph_and_search_contracts():
    with TestClient(create_app(TEACHING_DATASET_PATH)) as client:
        ...
        assert health.json()["node_count"] > 0
        ...
        graph = client.get("/api/v1/graph", params={"scenario": "incident"})
        ...
        body = response.json()
        assert body["found"]
        assert body["path"][0] == "emergency_115"
        assert body["path"][-1] == "family_hospital"
...
def test_validation_and_multi_route():
    with TestClient(create_app(TEACHING_DATASET_PATH)) as client:
        invalid = client.post(
            "/api/v1/search",
            json={"start_id": "missing", "goal_id": "dn_hospital"},
        )
        ...
        assert sorted(body["stop_order"]) == sorted(
            ["dn_hospital", "family_hospital", "son_tra_clinic"]
        )
        assert body["visit_sequence"][0] == body["visit_sequence"][-1] == "emergency_115"
